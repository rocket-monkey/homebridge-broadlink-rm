...
const startKeepAlive = (device, log) => {
  ...
  setInterval(() => {
    if (broadlink.debug) {
      log(
        "\x1b[33m[DEBUG]\x1b[0m Sending keepalive to",
        device.host.address,
        ":",
        device.host.port,
      );
    ...
    socket.send(
      ...
      (err, bytes) => {
        if (err) {
          log("\x1b[33m[DEBUG]\x1b[0m send keepalive packet error", err);
        }
      },
    );
    socket.close();
    ...
    try {
      ping.sys.probe(
        ...
        (active, err) => {
          if (err) {
            log(
              `Error pinging Broadlink RM device at ${device.host.address} (${device.host.macAddress || ""}): ${err}`,
            );
            throw err;
          }
          ...
          if (!active && device.state === "active" && device.retryCount === 2) {
            log(
              `Broadlink RM device at ${device.host.address} (${device.host.macAddress || ""}) is no longer reachable after three attempts.`,
            );
            ...
            if (broadlink.debug) {
              log(
                `Broadlink RM device at ${device.host.address} (${device.host.macAddress || ""}) is no longer reachable. (attempt ${device.retryCount})`,
              );
            }
            ...
            if (device.state === "inactive") {
              log(
                `Broadlink RM device at ${device.host.address} (${device.host.macAddress || ""}) has been re-discovered.`,
              );
            }
...
const discoverDevices = (
  ...
  broadlink.on("deviceReady", (device) => {
    ...
    log(
      `\x1b[35m[INFO]\x1b[0m Discovered ${device.model} (${device.type.toString(16)}) at ${device.host.address} (${device.host.macAddress})`,
    );
    addDevice(device);
...
const getDevice = ({ host, log, learnOnly }) => {
  ...
  if (host) {
    ...
    if (learnOnly) {
      ...
      if (!device) {
        log(`Learn Code (no device found at ${host})`);
      }
    } else {
      device = discoveredDevices[hosts[0]];

      if (!device) {
        log(`Send data (no device found at ${host})`);
      }
    }
  }
