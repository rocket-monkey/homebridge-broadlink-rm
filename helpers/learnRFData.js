...
const stop = (log, device, logLevel) => {
  ...
  if (!closeClient || isClosingClient) {
    return;
  }
  ...
  setTimeout(() => {
    ...
    if (log) {
      log(`\x1b[35m[INFO]\x1b[0m Scan RF (stopped)`);
    }
  }, 500);

  if (this.initalDebug !== undefined && currentDevice) {
    currentDevice.debug = this.initalDebug;
  }
};
...
const start = (
  ...
  if (!device) {
    return log(
      `\x1b[35m[INFO]\x1b[0m Learn Code (Couldn't learn code, device not found)`,
    );
  }
  ...
  if (!device.enterLearning) {
    return log(
      `\x1b[31m[ERROR]\x1b[0m Learn Code (IR/RF learning not supported for device at ${host})`,
    );
  }
  if (!device.enterRFSweep) {
    return log(
      `\x1b[31m[ERROR]\x1b[0m Scan RF (RF learning not supported for device (${device.type}) at ${host})`,
    );
  }
  ...
  onRawData2 = (message) => {
    ...
    log(
      `\x1b[35m[ACTION]\x1b[0m Press the RF button multiple times with a pause between them to get code.`,
    );
  ...
  log(
    `\x1b[35m[ACTION]\x1b[0m Hold down the button that sends the RF frequency.`,
  );
