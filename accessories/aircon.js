...
class AirConAccessory extends BroadlinkRMAccessory {
  constructor(log, config = {}, serviceManagerType) {
    ...
    HeatingCoolingConfigKeys[Characteristic.TargetHeatingCoolingState.OFF] =
      "off";
    HeatingCoolingConfigKeys[Characteristic.TargetHeatingCoolingState.COOL] =
      "cool";
    HeatingCoolingConfigKeys[Characteristic.TargetHeatingCoolingState.HEAT] =
      "heat";
    HeatingCoolingConfigKeys[Characteristic.TargetHeatingCoolingState.AUTO] =
      "auto";
  ...
  correctReloadedState(state) {
    if (
      state.currentHeatingCoolingState ===
  ...
  setDefaults() {
    ...
    if (config.turnOnWhenOff === undefined) {
      config.turnOnWhenOff = config.sendOnWhenOff || false;
    } // Backwards compatible with `sendOnWhenOff`
    if (config.minimumAutoOnOffDuration === undefined) {
      config.minimumAutoOnOffDuration = config.autoMinimumDuration || 120;
    } // Backwards compatible with `autoMinimumDuration`
    ...
    state.currentHeatingCoolingState =
      state.currentHeatingCoolingState ||
      Characteristic.CurrentHeatingCoolingState.OFF;
    state.targetHeatingCoolingState =
      state.targetHeatingCoolingState ||
    ...
    if (config.pseudoDeviceTemperature) {
      assert.isBelow(
        config.pseudoDeviceTemperature,
        config.maxTemperature + 1,
        `\x1b[31m[CONFIG ERROR] \x1b[33mpseudoDeviceTemperature\x1b[0m (${config.pseudoDeviceTemperature}) must be less than the maxTemperature (${config.maxTemperature})`,
      );
      assert.isAbove(
        config.pseudoDeviceTemperature,
        config.minTemperature - 1,
        `\x1b[31m[CONFIG ERROR] \x1b[33mpseudoDeviceTemperature\x1b[0m (${config.pseudoDeviceTemperature}) must be more than the minTemperature (${config.minTemperature})`,
      );
    }
    ...
    assert.isBelow(
      ...
      `\x1b[31m[CONFIG ERROR] \x1b[33mmaxTemperature\x1b[0m (${config.minTemperature}) must be more than minTemperature (${config.minTemperature})`,
    );
  }
  ...
  updateAccessories(accessories) {
    ...
    const autoSwitchAccessories = accessories.filter(
      (accessory) => accessory.name === autoSwitchName,
    );

    if (autoSwitchAccessories.length === 0) {
      return log(
        `${name} No accessory could be found with the name "${autoSwitchName}". Please update the "autoSwitchName" value or add a matching switch accessory.`,
      );
    }
  ...
  isAutoSwitchOn() {
    return (
      !this.autoSwitchAccessory ||
      (this.autoSwitchAccessory &&
        this.autoSwitchAccessory.state &&
        this.autoSwitchAccessory.state.switchState)
    );
  }
  ...
  setTargetTemperature(previousValue) {
    ...
    if (state.targetTemperature < minTemperature) {
      return log(
        `The target temperature (${this.targetTemperature}) must be more than the minTemperature (${minTemperature})`,
      );
    }
    if (state.targetTemperature > maxTemperature) {
      return log(
        `The target temperature (${this.targetTemperature}) must be less than the maxTemperature (${maxTemperature})`,
      );
    }
  ...
  async setTargetHeatingCoolingState(hexData, previousValue) {
    ...
    const targetHeatingCoolingState =
      HeatingCoolingConfigKeys[state.targetHeatingCoolingState];
    const lastUsedHeatingCoolingState =
      HeatingCoolingConfigKeys[state.lastUsedHeatingCoolingState];
    const currentHeatingCoolingState =
      HeatingCoolingConfigKeys[state.currentHeatingCoolingState];
    ...
    if (targetHeatingCoolingState === "off") {
      ...
      if (
        ...
        data.offDryMode !== undefined
      ) {
        // Dry off mode when previously cooling
        if (logLevel <= 2) {
          log(
            `${name} Previous state ${currentHeatingCoolingState}, setting off with dry mode`,
          );
        }
    ...
    if (this.checkTurnOnWhenOff()) {
      this.turnOnWhenOffDelayPromise = delayForDuration(0.3);
    ...
    if (replaceAutoMode && targetHeatingCoolingState === "auto") {
      if (logLevel <= 2) {
        log(
          `${name} setTargetHeatingCoolingState (converting from auto to ${replaceAutoMode})`,
        );
      }
      this.updateServiceTargetHeatingCoolingState(
        HeatingCoolingStates[replaceAutoMode],
      );
    ...
    if (state.currentHeatingCoolingState !== state.targetHeatingCoolingState) {
      ...
      this.updateServiceCurrentHeatingCoolingState(
        state.targetHeatingCoolingState,
      );
  ...
  async checkAutoOff() {
    await catchDelayCancelError(async () => {
      ...
      if (enableAutoOff && parseInt(onDuration) > 0) {
        log(
          `${name} setTargetHeatingCoolingState: (automatically turn off in ${onDuration} seconds)`,
        );
        if (this.autoOffTimeoutPromise) {
          this.autoOffTimeoutPromise.cancel();
          this.autoOffTimeoutPromise = null;
  ...
  async sendTemperature(temperature, previousTemperature) {
    ...
    if (logLevel <= 1) {
      log(
        `\x1b[34m[DEBUG]\x1b[0m ${name} Potential sendTemperature (${temperature})`,
      );
    }

    // Ignore Temperature if off, staying off - and set to ignore. OR temperature not provided
    if (
      (!state.targetHeatingCoolingState && ignoreTemperatureWhenOff) ||
      !temperature
    ) {
      if (logLevel <= 2) {
        log(
          `${name} Ignoring sendTemperature due to "ignoreTemperatureWhenOff": true or no temperature set.`,
        );
      }
      return;
    }
    ...
    let mode = HeatingCoolingConfigKeys[state.targetHeatingCoolingState];
    const { hexData, finalTemperature } = this.getTemperatureHexData(
      mode,
      temperature,
    );
    ...
    if (hexData["pseudo-mode"]) {
      ...
      if (mode) {
        assert.oneOf(
          mode,
          ["heat", "cool", "auto"],
          `\x1b[31m[CONFIG ERROR] \x1b[33mpseudo-mode\x1b[0m should be one of "heat", "cool" or "auto"`,
        );
      }
    ...
    if (
      previousTemperature !== finalTemperature ||
      (state.firstTemperatureUpdate && !preventResendHex)
    ) {
      //Set the temperature
  ...
  getTemperatureHexData(mode, temperature) {
    ...
    const { defaultHeatTemperature, defaultCoolTemperature, heatTemperature } =
      config;
    ...
    if (!hexData) {
      ...
      if (hexData["pseudo-mode"]) {
        if (logLevel <= 2) {
          this.log(
            `\x1b[36m[INFO] \x1b[0m${name} Configuration found for ${mode}${temperature} with pseudo-mode. Pseudo-mode will replace the configured mode.`,
          );
        }
      }
    }
    ...
    if (!hexData) {
      ...
      assert(
        hexData,
        `\x1b[31m[CONFIG ERROR] \x1b[0m You need to provide a hex code for the following temperature:
      ...
      if (logLevel <= 2) {
        this.log(
          `${name} Update to default temperature (${defaultTemperature})`,
        );
      }
  ...
  async checkTurnOnWhenOff() {
    ...
    if (
      ...
      if (on) {
        ...
        if (logLevel <= 4) {
          log(
            `\x1b[31m[CONFIG ERROR] \x1b[0m ${name} No On Hex configured, but turnOnWhenOff enabled`,
          );
        }
      }
  ...
  onTemperature(temperature, humidity) {
    ...
    if (logLevel <= 2) {
      log(`\x1b[36m[INFO] \x1b[0m${name} onTemperature (${temperature})`);
    }
    ...
    if (humidity) {
      if (noHumidity) {
        ...
        if (logLevel <= 2) {
          log(`\x1b[36m[INFO] \x1b[0m${name} onHumidity (` + humidity + `)`);
        }
      }
    }
  ...
  getCurrentTemperature(callback) {
    ...
    if (pseudoDeviceTemperature !== undefined) {
      if (logLevel <= 1) {
        log(
          `\x1b[34m[DEBUG]\x1b[0m ${name} getCurrentTemperature (using pseudoDeviceTemperature ${pseudoDeviceTemperature} from config)`,
        );
      }
  ...
  async checkTemperatureForAutoOnOff(temperature) {
    ...
    let { autoHeatTemperature, autoCoolTemperature, minimumAutoOnOffDuration } =
      config;

    if (this.shouldIgnoreAutoOnOff) {
      if (logLevel <= 2) {
        log(
          `${name} checkTemperatureForAutoOn (ignore within ${minimumAutoOnOffDuration}s of previous auto-on/off due to "minimumAutoOnOffDuration")`,
        );
      }
    ...
    if (!autoHeatTemperature && !autoCoolTemperature) {
      return;
    }

    if (!this.isAutoSwitchOn()) {
      if (logLevel <= 2) {
        log(`${name} checkTemperatureForAutoOnOff (autoSwitch is off)`);
      }
      return;
    }

    if (logLevel <= 1) {
      log(`${name} checkTemperatureForAutoOnOff`);
    }
    ...
    if (autoHeatTemperature && temperature < autoHeatTemperature) {
      ...
      if (logLevel <= 2) {
        log(
          `${name} checkTemperatureForAutoOnOff (${temperature} < ${autoHeatTemperature}: auto heat)`,
        );
      }
      serviceManager.setCharacteristic(
        Characteristic.TargetHeatingCoolingState,
      ...
      if (logLevel <= 2) {
        log(
          `${name} checkTemperatureForAutoOnOff (${temperature} > ${autoCoolTemperature}: auto cool)`,
        );
      }
      serviceManager.setCharacteristic(
        Characteristic.TargetHeatingCoolingState,
      ...
      if (logLevel <= 1) {
        log(`${name} checkTemperatureForAutoOnOff (temperature is ok)`);
      }

      if (this.state.isRunningAutomatically) {
        this.isAutomatedOff = true;
        if (logLevel <= 2) {
          log(`${name} checkTemperatureForAutoOnOff (auto off)`);
        }
        serviceManager.setCharacteristic(
          Characteristic.TargetHeatingCoolingState,
          Characteristic.TargetHeatingCoolingState.OFF,
    ...
    this.shouldIgnoreAutoOnOffPromise = delayForDuration(
