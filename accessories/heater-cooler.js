...
class HeaterCoolerAccessory extends BroadlinkRMAccessory {
  ...
  constructor(log, config = {}, serviceManagerType) {
    ...
    if (config.noHistory !== true) {
      ...
      this.historyService = new HistoryService("room", this, {
        storage: "fs",
  ...
  setDefaults() {
    ...
    if (
      config.preventResendHex === undefined &&
      config.allowResend === undefined
    ) {
    ...
    config.turnOnWhenOff =
      config.turnOnWhenOff === undefined ? true : config.turnOnWhenOff;

    state.active = state.active || Characteristic.Active.INACTIVE;
    state.currentHeaterCoolerState =
      state.currentHeaterCoolerState ||
      Characteristic.CurrentHeaterCoolerState.INACTIVE;

    if (state.coolingThresholdTemperature === undefined) {
      state.coolingThresholdTemperature = coolingThresholdTemperature;
    }
    if (state.heatingThresholdTemperature === undefined) {
      state.heatingThresholdTemperature = heatingThresholdTemperature;
    }
    if (state.targetHeaterCoolerState === undefined) {
      state.targetHeaterCoolerState =
        defaultMode === "cool"
          ? Characteristic.TargetHeaterCoolerState.COOL
          : Characteristic.TargetHeaterCoolerState.HEAT;
    }
    if (state.currentTemperature === undefined) {
      state.currentTemperature = config.defaultNowTemperature;
    }
    ...
    if (config.mqttURL) {
      ...
      config.temperatureUpdateFrequency =
        config.temperatureUpdateFrequency || 600;
    } else {
      config.temperatureUpdateFrequency =
        config.temperatureUpdateFrequency || 10;
    }
    ...
    if (available.cool.rotationSpeed || available.heat.rotationSpeed) {
      if (state.rotationSpeed === undefined) {
        state.rotationSpeed = defaultRotationSpeed;
      }
    }
    if (available.cool.swingMode || available.heat.swingMode) {
      if (state.swingMode === undefined) {
        state.swingMode = Characteristic.SwingMode.SWING_DISABLED;
      }
    }
  }
  ...
  updateServiceCurrentHeaterCoolerState() {
    ...
    if (logLevel <= 2) {
      log(
        `Updated currentHeaterCoolerState to ${state.currentHeaterCoolerState}`,
      );
    }
  }
  ...
  async setTargetHeaterCoolerState(hexData, previousValue) {
    ...
    if (logLevel <= 2) {
      log(
        `Changing target state from ${previousValue} to ${targetHeaterCoolerState}`,
      );
    }
    switch (targetHeaterCoolerState) {
      case Characteristic.TargetHeaterCoolerState.COOL:
        if (available.cool.temperatureCodes) {
          ...
          coolingThresholdTemperature = this.serviceManager.getCharacteristic(
            Characteristic.CoolingThresholdTemperature,
          ).value;
        }

        hexData = this.decodeHexFromConfig(
          CharacteristicName.TARGET_HEATER_COOLER_STATE,
        );
        break;
      case Characteristic.TargetHeaterCoolerState.HEAT:
        if (available.heat.temperatureCodes) {
          // update internal state to be consistent with what Home app & homebridge see
          heatingThresholdTemperature = this.serviceManager.getCharacteristic(
      ...
      default:
        if (logLevel <= 4) {
          log(
            `BUG: ${this.name} setTargetHeaterCoolerState invoked with unsupported target mode ${targetHeaterCoolerState}`,
          );
        }
    }
    ...
    switch (targetHeaterCoolerState) {
      case Characteristic.TargetHeaterCoolerState.COOL:
        ...
        if (!available.coolMode) {
          if (logLevel <= 4) {
            log(
              `BUG: ${name} decodeHexFromConfig invoked with unsupported target mode: cool.`,
            );
          }
          return "0'"; // sending dummy hex data to prevent homebridge from tripping
        }
        if (
          toUpdateCharacteristic === CharacteristicName.ACTIVE &&
        ...
        return this.decodeTemperatureHex(
          temperature,
          cool,
          toUpdateCharacteristic,
        );
        break;
      ...
      case Characteristic.TargetHeaterCoolerState.HEAT:
        ...
        if (!available.heatMode) {
          if (logLevel <= 4) {
            log(
              `BUG: ${name} decodeHexFromConfig invoked with unsupported target mode: heat.`,
            );
          }
          return "0'"; // sending dummy hex data to prevent homebridge from tripping
        }
      ...
      default:
        if (logLevel <= 4) {
          log(
            `BUG: decodeHexFromConfig has invalid value for targetHeaterCoolerState: ${targetHeaterCoolerState}.`,
          );
        }
        break;
    }
   ...
   * @returns {any} hexData - object, array or string values to be sent to IR device
   */
  decodeHierarchichalHex(
    hexDataObject,
    checkCharacteristics,
    toUpdateCharacteristic,
  ) {
    const { state, log, logLevel, name } = this;
    if (hexDataObject === undefined || hexDataObject == null) {
      return "hexDataObject";
    } // should never happen, unless bug
    if (typeof hexDataObject !== "object") {
      return hexDataObject;
    }
    if (Array.isArray(hexDataObject)) {
      return hexDataObject;
    }
    ...
    switch (characteristic) {
      case CharacteristicName.ROTATION_SPEED:
        ...
        if (toUpdateCharacteristic === CharacteristicName.ROTATION_SPEED) {
          if (keys.includes("fanSpeedToggle")) {
            return this.decodeHierarchichalHex(
              hexDataObject.fanSpeedToggle,
              checkCharacteristics,
              null,
            );
          }
      ...
      case CharacteristicName.SWING_MODE:
        if (toUpdateCharacteristic === CharacteristicName.SWING_MODE) {
          if (keys.includes("swingToggle")) {
            return this.decodeHierarchichalHex(
              hexDataObject.swingToggle,
              checkCharacteristics,
              null,
            );
          }
          keyFromState =
            state.swingMode === Characteristic.SwingMode.SWING_ENABLED
              ? "swingOn"
              : "swingOff";
          if (keys.includes(keyFromState)) {
            return this.decodeHierarchichalHex(
          ...
          if (logLevel <= 3) {
            log(
              `Could not find swingMode hex codes for swingMode ${keyFromState}`,
            );
          }
          return "0";
        }
      ...
      case undefined:
        ...
        if (logLevel <= 4) {
          log(
            `BUG: ${name} decodeHierarchichalHex encountered a bug, please raise an issue`,
          );
        }
        return hexDataObject;
    }
  ...
  decodeTemperatureHex(temperature, hexDataObject, toUpdateCharacteristic) {
    ...
    if (logLevel <= 2) {
      log(`Looking up temperature hex codes for ${temperature}`);
    }
    ...
    if (temperatureHexDataObject) {
      hexCode = this.decodeHierarchichalHex(
        temperatureHexDataObject,
        CONFIG_CHARACTERISTICS,
        toUpdateCharacteristic,
      );
      if (logLevel <= 2) {
        log(`\tSending hex codes for temperature ${temperature}`);
      }
    } else {
      if (logLevel <= 4) {
        log(
          `\tDid not find temperature code for ${temperature}. Please update data.${
            this.state.targetHeaterCoolerState === 1 ? "heat" : "cool"
          }.temperatureCodes in config.json`,
        );
      }
    }
  ...
  async setTemperature(hexData, previousValue) {
    ...
    if (logLevel <= 2) {
      log(
        `${name} setTemperature: Changing temperature from ${previousValue} to ${targetTemperature}`,
      );
    }
    hexData = this.decodeHexFromConfig(
      targetHeaterCoolerState === Characteristic.TargetHeaterCoolerState.COOL
        ? CharacteristicName.CoolingThresholdTemperature
        : CharacteristicName.HeatingThresholdTemperature,
    );
  ...
  async setActive(hexData, previousValue) {
    ...
    if (
      ...
      previousValue === Characteristic.Active.INACTIVE
    ) {
  ...
  async setSwingMode(hexData, previousValue) {
    ...
    if (logLevel <= 2) {
      log(
        `${name} setSwingMode: Changing swing from ${previousValue} to ${Characteristic.SwingMode.SWING_ENABLED}`,
      );
    }
    ...
    if (hexData === "0") {
      if (logLevel <= 3) {
        log(`Swing hex codes not found, resetting state to previous value`);
      }
  ...
  async setRotationSpeed(hexData, previousValue) {
    ...
    if (logLevel <= 2) {
      log(
        `${name} setRotationSpeed: Changing RotationSpeed from ${previousValue} to ${state.rotationSpeed}`,
      );
    }
    ...
    if (hexData === "0") {
      if (logLevel <= 3) {
        log(`Fan speed hex codes not found, resetting back to previous value`);
      }
      state.rotationSpeed = previousValue;
      this.serviceManager.service
        .getCharacteristic(Characteristic.RotationSpeed)
  ...
  async monitorTemperature() {
    ...
    if (w1DeviceID || temperatureFilePath) {
      config.temperatureUpdateFrequency = Math.max(
        config.temperatureUpdateFrequency,
        60,
      );
    }
    ...
    if (!config.isUnitTest) {
      setInterval(
        this.updateTemperatureUI.bind(this),
        config.temperatureUpdateFrequency * 1000,
      );
    }
  }
  ...
  onTemperature(temperature, humidity) {
    ...
    if (Object.keys(this.temperatureCallbackQueue).length === 0) {
      return;
    }

    temperature += temperatureAdjustment;
    if (tempSourceUnits == "F") {
      temperature = ((temperature - 32) * 5) / 9;
    }
    state.currentTemperature = temperature;
    if (logLevel <= 1) {
      log(`\x1b[34m[DEBUG]\x1b[0m ${name} onTemperature (${temperature})`);
    }
    ...
    if (humidity) {
      if (noHumidity) {
        ...
        if (logLevel <= 1) {
          log(`\x1b[34m[DEBUG]\x1b[0m ${name} onHumidity (` + humidity + `)`);
        }
      }
    }
    ...
    if (config.noHistory !== true && this.state.currentTemperature != 0.0) {
      this.lastUpdatedAt = Date.now();
      if (logLevel <= 1) {
        log(
          `\x1b[34m[DEBUG]\x1b[0m ${name} Logging data to history: temp: ${this.state.currentTemperature}, humidity: ${this.state.currentHumidity}`,
        );
      }
      if (noHumidity) {
  ...
  addTemperatureCallbackToQueue(callback) {
    ...
    if (Object.keys(this.temperatureCallbackQueue).length > 1) {
      if (state.currentTemperature) {
        if (logLevel <= 1) {
          log(
            `\x1b[34m[DEBUG]\x1b[0m ${name} addTemperatureCallbackToQueue (clearing previous callback, using existing temperature)`,
          );
        }
    ...
    if (mqttURL) {
      ...
      const humidity = noHumidity
        ? null
        : this.mqttValueForIdentifier("humidity");
    ...
    if (!device || device.state === "inactive") {
      if (device && device.state === "inactive") {
        if (logLevel <= 3) {
          log(
            `${name} addTemperatureCallbackToQueue (device no longer active, using existing temperature)`,
          );
        }
      }
    ...
    if (logLevel < 1) {
      log(
        `\x1b[34m[DEBUG]\x1b[0m ${name} addTemperatureCallbackToQueue (requested temperature from device, waiting)`,
      );
    }
  }
  ...
  updateTemperatureFromFile() {
    ...
    if (logLevel <= 1) {
      log(
        `\x1b[34m[DEBUG]\x1b[0m ${name} updateTemperatureFromFile reading file: ${temperatureFilePath}`,
      );
    }
    ...
    fs.readFile(temperatureFilePath, "utf8", (err, data) => {
      if (err) {
        if (logLevel <= 4) {
          log(
            `\x1b[31m[ERROR] \x1b[0m${name} updateTemperatureFromFile\n\n${err.message}`,
          );
        }
      }
      ...
      if (data === undefined || data.trim().length === 0) {
        if (logLevel <= 3) {
          log(
            `\x1b[33m[WARNING]\x1b[0m ${name} updateTemperatureFromFile error reading file: ${temperatureFilePath}, using previous Temperature`,
          );
        }
        if (!noHumidity) {
        ...
        lines.forEach((line) => {
          if (-1 < line.indexOf(":")) {
            ...
            if (value[0] == "temperature") {
              temperature = parseFloat(value[1]);
            }
            if (value[0] == "humidity" && !noHumidity) {
              humidity = parseFloat(value[1]);
            }
            if (value[0] == "battery" && batteryAlerts) {
              state.batteryLevel = parseFloat(value[1]);
            }
          }
        });
      }
      ...
      if (logLevel <= 1) {
        log(
          `\x1b[34m[DEBUG]\x1b[0m ${name} updateTemperatureFromFile (parsed temperature: ${temperature} humidity: ${humidity})`,
        );
      }
  ...
  updateTemperatureFromW1() {
    ...
    if (logLevel <= 1) {
      log(
        `\x1b[34m[DEBUG]\x1b[0m ${name} updateTemperatureFromW1 reading file: ${fName}`,
      );
    }
    ...
    fs.readFile(fName, "utf8", (err, data) => {
      if (err) {
        if (logLevel <= 4) {
          log(
            `\x1b[31m[ERROR] \x1b[0m${name} updateTemperatureFromW1\n\n${err.message}`,
          );
        }
      }
      ...
      if (data.includes("t=")) {
        ...
        if (logLevel <= 4) {
          log(
            `\x1b[33m[WARNING]\x1b[0m ${name} updateTemperatureFromW1 error reading file: ${fName}, using previous Temperature`,
          );
        }
        temperature = state.currentTemperature || 0;
      }
      ...
      if (logLevel <= 1) {
        log(
          `\x1b[34m[DEBUG]\x1b[0m ${name} updateTemperatureFromW1 (parsed temperature: ${temperature})`,
        );
      }
  ...
  getCurrentTemperature(callback) {
    ...
    if (defaultNowTemperature !== undefined) {
      if (logLevel <= 1) {
        log(
          `\x1b[34m[DEBUG]\x1b[0m ${name} getCurrentTemperature (using defaultNowTemperature ${defaultNowTemperature} from config)`,
        );
      }
  ...
  onMQTTMessage(identifier, message) {
    ...
    if (
      ...
      if (logLevel <= 4) {
        log(
          `\x1b[31m[ERROR] \x1b[0m${name} onMQTTMessage (mqtt message received with unexpected identifier: ${identifier}, ${message.toString()})`,
        );
      }
    ...
    if (logLevel <= 1) {
      log(`\x1b[34m[DEBUG]\x1b[0m ${name} onMQTTMessage (raw value: ${value})`);
    }
    try {
      ...
      if (typeof temperatureJSON === "object") {
        ...
        if (identifier !== "temperature" && identifier !== "battery") {
          ...
          if (values.length === 0) {
            values = findKey(temperatureJSON, "Hum");
          }
          if (values.length === 0) {
            values = findKey(temperatureJSON, "hum");
          }
          if (values.length === 0) {
            values = findKey(temperatureJSON, "Humidity");
          }
          if (values.length === 0) {
            values = findKey(temperatureJSON, "humidity");
          }
          if (values.length === 0) {
            values = findKey(temperatureJSON, "RelativeHumidity");
          }
          if (values.length === 0) {
            values = findKey(temperatureJSON, "relativehumidity");
          }
        ...
        if (identifier !== "temperature" && identifier !== "humidity") {
          ...
          if (values.length === 0) {
            values = findKey(temperatureJSON, "Batt");
          }
          if (values.length === 0) {
            values = findKey(temperatureJSON, "batt");
          }
          if (values.length === 0) {
            values = findKey(temperatureJSON, "Battery");
          }
          if (values.length === 0) {
            values = findKey(temperatureJSON, "battery");
          }
        ...
        if (identifier !== "battery" && identifier !== "humidity") {
          ...
          if (values.length === 0) {
            values = findKey(temperatureJSON, "temp");
          }
          if (values.length === 0) {
            values = findKey(temperatureJSON, "Temp");
          }
          if (values.length === 0) {
            values = findKey(temperatureJSON, "temperature");
          }
          if (values.length === 0) {
            values = findKey(temperatureJSON, "Temperature");
          }
          if (values.length === 0) {
            values = findKey(temperatureJSON, "local_temperature");
          }
          if (values.length > 0) {
    ...
    if (objectFound) {
      ...
      if (
        ...
        if (logLevel <= 3) {
          log(
            `\x1b[31m[ERROR] \x1b[0m${name} onMQTTMessage (mqtt value not found)`,
          );
        }
        return;
      }
      ...
      if (logLevel <= 1) {
        log(
          `\x1b[34m[DEBUG]\x1b[0m ${name} onMQTTMessage (parsed value: ${value})`,
        );
      }
  ...
  validateOptionalCharacteristics(dataObject, configObject) {
    const isValidRotationSpeedKey = (stringValue) =>
      stringValue.startsWith("rotationSpeed"); // loose validation, can further validate number endings
    const isValidSwingModeKey = (stringValue) =>
      stringValue.startsWith("swing");
    ...
    if (
      !configObject.temperatureCodes &&
      dataObjectKeys.every(isValidTemperature)
    ) {
      configObject.temperatureCodes = true;
    } else if (
      !configObject.rotationSpeed &&
      dataObjectKeys.every(isValidRotationSpeedKey)
    ) {
      configObject.rotationSpeed = true;
    } else if (
      !configObject.swingMode &&
      dataObjectKeys.every(isValidSwingModeKey)
    ) {
  ...
  configureOptionalCharacteristics() {
    ...
    const { heat, cool } = data;
    assert(
      available.coolMode || available.heatMode,
      `ERROR: ${name} configureOptionalCharacteristics invoked without configuring heat and cool modes`,
    );
    ...
    if (
      available.coolMode &&
      cool.temperatureCodes &&
      typeof cool.temperatureCodes === "object" &&
      !Array.isArray(cool.temperatureCodes)
    ) {
      this.validateOptionalCharacteristics(
        cool.temperatureCodes,
        available.cool,
      );
    }
    ...
    if (
      available.heatMode &&
      heat.temperatureCodes &&
      typeof heat.temperatureCodes === "object" &&
  ...
  configureTemperatures() {
    ...
    if (!["C", "c", "F", "f"].includes(config.temperatureUnits)) {
    ...
    if (coolingThresholdTemperature === undefined) {
      ...
      config.coolingThresholdTemperature = this.temperatureFtoC(
        coolingThresholdTemperature,
      );
    }
    if (heatingThresholdTemperature === undefined) {
      ...
      config.heatingThresholdTemperature = this.temperatureFtoC(
        heatingThresholdTemperature,
      );
    }
    //if (defaultNowTemperature === undefined) {
    ...
    if (temperatureUnits === "f") {
      if (config.minTemperature) {
        config.minTemperature = this.temperatureFtoC(config.minTemperature);
      }
      if (config.maxTemperature) {
        config.maxTemperature = this.temperatureFtoC(config.maxTemperature);
      }
    }
    ...
    if (available.heatMode) {
      heat.minTemperature = Math.min(
        heat.minTemperature,
        HEATING_THRESHOLD_TEMPERATURE.minValue,
      );
      heat.maxTemperature = Math.max(
        heat.maxTemperature,
        HEATING_THRESHOLD_TEMPERATURE.maxValue,
      );
    }

    if (available.coolMode) {
      cool.minTemperature = Math.min(
        cool.minTemperature,
        COOLING_THRESHOLD_TEMPERATURE.minValue,
      );
      cool.maxTemperature = Math.max(
        cool.maxTemperature,
        COOLING_THRESHOLD_TEMPERATURE.maxValue,
      );
    }
  }
  ...
  configureHeatCoolModes() {
    ...
    const { internalConfig } = config;
    assert(
      internalConfig !== undefined && typeof internalConfig === "object",
      `ERROR: ${this.name} internalConfig is not initialized. Please raise an issue`,
    );
    const { available } = internalConfig;
    assert(
      available !== undefined && typeof available === "object",
      `ERROR: ${this.name} internalConfig.available is not initialized. Please raise an issue`,
    );
    ...
    if (!available.coolMode && !available.heatMode) {
      throw new Error(
        `At least one of data.cool or data.heat object is required in config.json. Please update your config.json file`,
      );
    }
  ...
  configDefaultsHelper() {
    ...
    if (config === undefined || typeof config !== "object") {
      throw new Error(
        "config.json is not setup properly, please check documentation",
      );
    }
    ...
    if (data === undefined || typeof data !== "object") {
      throw new Error(
        `data object is required in config.json for initializing accessory`,
      );
    }
