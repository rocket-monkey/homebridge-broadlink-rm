...
class GarageDoorOpenerAccessory extends BroadlinkRMAccessory {
  ...
  async setDoorTargetState(hexData, previousValue) {
    ...
    if (
      previousValue !== state.doorTargetState &&
      state.doorCurrentState === state.doorTargetState
    ) {
      serviceManager.setCharacteristic(
        Characteristic.CurrentDoorState,
        !state.doorTargetState,
      );
    }
  ...
  async open(hexData) {
    ...
    // Defaults
    if (!openDuration) {
      openDuration = openCloseDuration || 8;
    }
  ...
  async close() {
    ...
    // Defaults
    if (!closeDuration) {
      closeDuration = openCloseDuration || 8;
    }
