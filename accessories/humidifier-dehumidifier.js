...
class HumidifierDehumidifierAccessory extends FanAccessory {
  ...
  async setCurrentState(hexData, previousValue) {
    ...
    if (logLevel <= 1) {
      log(
        `\x1b[34m[DEBUG]\x1b[0m ${name} setCurrentState: requested update from ${previousValue} to ${state.currentState}`,
      );
    }

    // Ignore if no change to the targetPosition
    if (state.currentState === previousValue || !state.switchState) {
      return;
    }
    ...
    if (logLevel <= 2) {
      log(
        `${name} setCurrentState: currently ${previousValue}, changing to ${state.currentState}`,
      );
    }
  ...
  async setHumidifierThreshold(hexData, previousValue) {
    ...
    if (
      state.HumidifierThreshold === previousValue &&
      config.preventResendHex &&
      !this.previouslyOff
    ) {
      return;
    }
    ...
    if (state.currentState === desiredState) {
      return;
    }

    if (logLevel <= 2) {
      log(
        `${name} setHumidifierThreshold: currently ${previousValue} to ${state.DehumidifierThreshold}, changing to ${state.HumidifierThreshold} to ${state.DehumidifierThreshold}`,
      );
    }
  ...
  async setDehumidifierThreshold(hexData, previousValue) {
    const { config, name, log, state, logLevel } = this;
    if (
      state.DehumidifierThreshold === previousValue &&
      config.preventResendHex &&
      !this.previouslyOff
    ) {
      return;
    ...
    if (logLevel <= 2) {
      log(
        `${name} setDeumidifierThreshold: currently ${state.HumidifierThreshold} to ${previousValue}, changing to ${state.HumidifierThreshold} to ${state.DehumidifierThreshold}`,
      );
    }
    state.currentState = desiredState;
    this.setCurrentState(hexData, previousState);
  ...
  getDesiredState() {
    ...
    if (
      ...
      if (
        state.currentHumidity > state.HumidifierThreshold &&
        state.currentHumidity < state.DehumidifierThreshold
      ) {
  ...
  async updateDeviceState() {
    ...
    if (
      state.targetState === Characteristic.TargetHumidifierDehumidifierState.OFF
    ) {
      state.currentState =
        Characteristic.CurrentHumidifierDehumidifierState.INACTIVE;
    ...
    if (state.currentState === desiredState && !this.previouslyOff) {
      return;
    }
    ...
    if (logLevel <= 1) {
      log(
        `\x1b[34m[DEBUG]\x1b[0m ${name} updateDeviceState: currently ${state.currentState}, changing to ${desiredState}`,
      );
    }
  ...
  async monitorHumidity() {
    ...
    if (logLevel <= 1) {
      log(`\x1b[34m[DEBUG]\x1b[0m ${name} monitorHumidity`);
    }
    ...
    if (!config.isUnitTest && !config.noHumidity) {
      setInterval(
  ...
  onHumidity(temperature, humidity) {
    ...
    if (Object.keys(this.humidityCallbackQueue).length === 0) {
      return;
    }
    ...
    if (config.noHistory !== true && this.state.currentHumidity != 0) {
      ...
      if (logLevel <= 1) {
        log(
          `\x1b[34m[DEBUG]\x1b[0m ${name} Logging data to history: humidity: ${this.state.currentHumidity}`,
        );
      }
  ...
  addHumidityCallbackToQueue(callback) {
    ...
    if (Object.keys(this.humidityCallbackQueue).length > 1) {
      if (state.currentHumidity) {
        if (logLevel <= 1) {
          log(
            `\x1b[34m[DEBUG]\x1b[0m ${name} addHumidityCallbackToQueue (clearing previous callback, using existing humidity)`,
          );
        }
    ...
    if (!device || device.state === "inactive") {
      if (device && device.state === "inactive") {
        if (logLevel <= 3) {
          log(
            `${name} addHumidityCallbackToQueue (device no longer active, using existing humidity)`,
          );
        }
      }
    ...
    if (logLevel < 1) {
      log(
        `\x1b[34m[DEBUG]\x1b[0m ${name} addHumidityCallbackToQueue (requested humidity from device, waiting)`,
      );
    }
  }
  ...
  updateHumidityFromFile() {
    ...
    if (logLevel <= 1) {
      log(
        `\x1b[34m[DEBUG]\x1b[0m ${name} updateHumidityFromFile reading file: ${humidityFilePath}`,
      );
    }
    ...
    fs.readFile(humidityFilePath, "utf8", (err, data) => {
      if (err) {
        if (logLevel <= 4) {
          log(
            `\x1b[31m[ERROR] \x1b[0m${name} updateHumidityFromFile\n\n${err.message}`,
          );
        }
      }
      ...
      if (data === undefined || data.trim().length === 0) {
        if (logLevel <= 3) {
          log(
            `\x1b[33m[WARNING]\x1b[0m ${name} updateHumidityFromFile error reading file: ${humidityFilePath}, using previous Temperature`,
          );
        }
      ...
      if (logLevel >= 4) {
        log(
          `\x1b[34m[DEBUG]\x1b[0m ${name} updateHumidityFromFile (parsed humidity: ${humidity})`,
        );
      }
    ...
    try {
      ...
      if (typeof humidityJSON === "object") {
        ...
        if (identifier !== "humidity") {
          ...
          if (values.length === 0) {
            values = findKey(humidityJSON, "Batt");
          }
          if (values.length === 0) {
            values = findKey(humidityJSON, "batt");
          }
          if (values.length === 0) {
            values = findKey(humidityJSON, "Battery");
          }
          if (values.length === 0) {
            values = findKey(humidityJSON, "battery");
          }
        ...
        if (identifier !== "battery") {
          ...
          if (values.length === 0) {
            values = findKey(humidityJSON, "Hum");
          }
          if (values.length === 0) {
            values = findKey(humidityJSON, "hum");
          }
          if (values.length === 0) {
            values = findKey(humidityJSON, "Humidity");
          }
          if (values.length === 0) {
            values = findKey(humidityJSON, "humidity");
          }
          if (values.length === 0) {
            values = findKey(humidityJSON, "RelativeHumidity");
          }
          if (values.length === 0) {
            values = findKey(humidityJSON, "relativehumidity");
          }
