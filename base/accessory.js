...
class HomebridgeAccessory {
  ...
  subscribeToMQTT() {
    ...
    if (typeof mqttTopic !== "string" && !Array.isArray(mqttTopic)) {
      if (this.logLevel <= 4) {
        log(
          `\x1b[31m[CONFIG ERROR]\x1b[0m ${name} \x1b[33mmqttTopic\x1b[0m value is incorrect. Please check out the documentation for more details.`,
        );
      }
  ...
  mqttValueForIdentifier(identifier) {
    ...
    if (value === undefined) {
      value = this.mqttValues.unknown;
    }

    if (!this.mqttClient.connected) {
      if (!this.isMQTTConnecting && logLevel <= 4) {
        log(
          `\x1b[31m[ERROR]\x1b[0m ${name} MQTT client is not connected. Value could not be found for topic with identifier "${identifier}".`,
        );
      }
    ...
    if (value === undefined) {
      if (this.logLevel <= 4) {
        log(
          `\x1b[31m[ERROR]\x1b[0m ${name} No MQTT value could be found for topic with identifier "${identifier}".`,
        );
      }
