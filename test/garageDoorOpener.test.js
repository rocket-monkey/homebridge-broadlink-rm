...
describe("doorAccessory", () => {
  ...
  it('"closeDuration": 0.2, closing -> closed', async () => {
    ...
    const doorAccessory = new GarageDoorOpener(
      null,
      config,
      "FakeServiceManager",
    );
    doorAccessory.serviceManager.setCharacteristic(
    ...
    expect(doorAccessory.state.doorTargetState).to.equal(
      Characteristic.TargetDoorState.CLOSED,
    );
    ...
    // Closed
    expect(doorAccessory.state.doorCurrentState).to.equal(
  ...
  it('"resendHexAfterReload": true, "persistState": true', async () => {
    ...
    await delayForDuration(0.3);
    expect(doorAccessory.serviceManager.hasRecordedSetCharacteristic).to.equal(
      true,
    );
  ...
  it('correctReloadedState for interupted open - "persistState": true', async () => {
    ...
    expect(doorAccessory.state.lockTargetState).to.equal(
      Characteristic.LockTargetState.SECURED,
    );
    ...
    expect(doorAccessory.state.lockCurrentState).to.equal(
      Characteristic.LockCurrentState.UNSECURED,
    );
  ...
  it("lock", async () => {
    ...
    const lockAccessory = new GarageDoorOpener(
      null,
      config,
      "FakeServiceManager",
    );
    lockAccessory.serviceManager.setCharacteristic(
      Characteristic.LockTargetState,
      Characteristic.LockTargetState.SECURED,
    );
    ...
    // Locked
    expect(lockAccessory.state.lockCurrentState).to.equal(
      Characteristic.LockCurrentState.SECURED,
    );
    expect(lockAccessory.state.lockTargetState).to.equal(
      Characteristic.LockTargetState.SECURED,
    );
