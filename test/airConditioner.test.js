...
describe("airConAccessory", async () => {
  ...
  it("tun on", async () => {
    ...
    airConAccessory.serviceManager.setCharacteristic(
    ...
    expect(airConAccessory.state.targetHeatingCoolingState).to.equal(
  ...
  it('"pseudoDeviceTemperature": 2', async () => {
    ...
    const getTemperaturePromise = airConAccessory.serviceManager
  ...
  it("autoSwitch", async () => {
    ...
    const airConAccessory = new AirCon(null, config, "FakeServiceManager");
    const switchAccessory = new Switch(
      null,
      switchConfig,
      "FakeServiceManager",
    );
